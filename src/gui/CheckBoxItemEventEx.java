...
public class CheckBoxItemEventEx extends JFrame {
    ...
    CheckBoxItemEventEx() {
        ...
        setSize(800,800);
        setVisible(true);
        panel2.add(label1);    //contenPane에 label부착하기
        panel2.add(label2);
        panel2.add(label3);
        panel2.add(label4);
        panel2.add(label5);

        label1.setIcon(img[0]);     //label에 이미지 부착하기
        label2.setIcon(img[1]);
        label3.setIcon(img[2]);
        label4.setIcon(img[3]);
        label5.setIcon(img[4]);

        label1.setVisible(false);   //label을 보이지 않게 디폴트값 설정
    ...
    class MyItemListener implements ItemListener {
        public void itemStateChanged(ItemEvent e) {
            int selected=1;
            if(e.getStateChange() == ItemEvent.SELECTED)    //이벤트가 선택되었을 때
                selected = 1;
            else
                selected = -1;
            /*--------------------------------------*/
            if(e.getItem() == fruits[0]) {  //이벤트가 발생했을 때 그냥 사과.
                sum = sum + selected * prices[0];  // 직접 가격값을 쓰지말고 price 배열을 이용하시오.
                if (selected == 1) {    //사과가 체크됬을때
